...
class TournamentController extends AbstractController
{
    ...
    public function tournamentList(
        ...
        } elseif ($request->get('tournament_id')) {
    ...
    #[Route('/tournament/refresh', name: 'app_tournament_refresh', methods: ['POST'])]
    public function refreshTournamentAction(
        Request $request,
        EntityManagerInterface $entityManager,
        TournamentRepository $tournamentRepository
    ): Response {
    ...
    public function refreshTournament(PlayoffMatch $match): ?PlayoffMatch
    {
        ...
        if ($scoreB1 < $scoreG1) {
            $match->setGreenTeam($match1->getGreenTeam());
        }
        if ($scoreB1 > $scoreG1) {
            $match->setGreenTeam($match1->getBlueTeam());
        }
        if ($scoreB2 < $scoreG2) {
            $match->setBlueTeam($match2->getGreenTeam());
        }
        if ($scoreB2 > $scoreG2) {
            $match->setBlueTeam($match2->getBlueTeam());
        }
